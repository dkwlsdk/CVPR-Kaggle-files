...
OUTPUT_DIR = "/root/Desktop/workspace/ja/qwen_test_output_9B/09-2.frame64_fps2_no_metadata"
...
RAW_LOG_DIR = os.path.join(OUTPUT_DIR, "raw_outputs")
...
def strip_thinking_text(text: str) -> str:
    ...
    text = re.sub(
        r'^(?:The user wants|I understand|Okay|Let me|Based on).*?:\s*',
        '',
        text,
...
def normalize_json_candidate(candidate: str) -> List[str]:
    """깨진 JSON 후보를 여러 버전으로 보정해 파싱 성공 확률을 높입니다."""
    base_candidate = str(candidate or '').strip()
    ...
    def merge_adjacent_strings(value: str) -> str:
        pattern = re.compile(r'"((?:[^"\\]|\\.)*)"\s+"((?:[^"\\]|\\.)*)"')
        previous = None
        while value != previous:
            previous = value
            value = pattern.sub(lambda m: '"' + m.group(1).rstrip() + ' ' + m.group(2).lstrip() + '"', value)
        return value
    ...
    for seed in list(variants):
        ...
        fixed = merge_adjacent_strings(fixed)
        add(fixed)

        if '\\"' in fixed:
            unescaped = fixed.replace('\\"', '"')
            unescaped = merge_adjacent_strings(unescaped)
            add(unescaped)
            add(re.sub(r'""(\s*[,}])', r'"\1', unescaped))

        boundary_fixed = re.sub(r'(:\s*)\\"', r'\1"', fixed)
        boundary_fixed = re.sub(r'\\"(\s*[,}])', r'"\1', boundary_fixed)
        boundary_fixed = re.sub(r'""(\s*[,}])', r'"\1', boundary_fixed)
        boundary_fixed = re.sub(r',\s*""\s*([}\]])', r'\1', boundary_fixed)
        boundary_fixed = re.sub(r',\s*"[^"]*"\s*([}\]])', r'\1', boundary_fixed)
        boundary_fixed = re.sub(r',(\s*[}\]])', r'\1', boundary_fixed)
        boundary_fixed = merge_adjacent_strings(boundary_fixed)
        add(boundary_fixed)
...
def try_parse_single_json(candidate: str) -> Optional[Dict[str, Any]]:
    """문자열을 JSON 딕셔너리로 파싱을 시도하며, 불완전하게 끊긴 경우 보정을 시도합니다."""
    variants = normalize_json_candidate(candidate)
    ...
    # 2. 닫는 중괄호가 누락된 경우를 대비해 하나씩 붙여보며 재시도 (최대 3개까지)
    for variant in variants:
        if variant.strip().startswith('{'):
            for i in range(1, 4):
                try:
                    fixed = variant.strip() + ("}" * i)
...
def extract_first_json_object(text: str) -> Optional[Dict[str, Any]]:
    """텍스트에서 유효한 첫 번째 JSON 객체를 추출합니다. 끊긴 JSON에 대해서도 유연하게 대응합니다."""
    ...
    # 2. 만약 완벽한 블록이 없거나 파싱 실패할 경우, 마지막 '{' 부터 문자열 끝까지를 후보로 추가 (불완전 중단 JSON 대응)
    last_open_brace = text.rfind('{')
    if last_open_brace != -1:
        candidates.append(text[last_open_brace:])
...
def sanitize_rel_path_for_log_filename(rel_path: Optional[str]) -> str:
    rel = str(rel_path or "unknown_video").strip().replace('\\', '/')
    rel = rel.lstrip('./')
    stem = os.path.splitext(rel)[0]
    safe = re.sub(r'[^A-Za-z0-9._-]+', '__', stem).strip('._')
    return safe or "unknown_video"

def get_raw_log_path(rel_path: Optional[str]) -> str:
    return os.path.join(RAW_LOG_DIR, f"{sanitize_rel_path_for_log_filename(rel_path)}.jsonl")

def reset_raw_logs() -> None:
    os.makedirs(RAW_LOG_DIR, exist_ok=True)
    for name in os.listdir(RAW_LOG_DIR):
        if not name.endswith('.jsonl'):
            continue
        file_path = os.path.join(RAW_LOG_DIR, name)
...
def write_run_summary(summary: Dict[str, Any]) -> None:
    summary["updated_at"] = current_utc_iso()
    temp_path = RUN_SUMMARY_PATH + ".tmp"
    with open(temp_path, "w", encoding="utf-8") as f:
        json.dump(summary, f, ensure_ascii=False, indent=2)
    os.replace(temp_path, RUN_SUMMARY_PATH)

def build_run_summary(total_rows: int, active_video_dir: str, metadata_rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    matched_paths = sum(1 for row in metadata_rows if row.get("path"))
    return {
        "status": "initializing",
        ...
        "execution": {
            ...
            "input_video_dir": active_video_dir,
            ...
            "matched_metadata_paths": matched_paths,
        },
        "hyperparameters": {
            ...
            "repetition_penalty": REPETITION_PENALTY,
            "video_fps": VIDEO_FPS,
            "max_video_sampled_frames": MAX_VIDEO_SAMPLED_FRAMES,
            "min_pixels": MIN_PIXELS,
            "max_pixels": MAX_PIXELS,
            "time_stage_max_pixels": TIME_STAGE_MAX_PIXELS,
            "max_time_retries": MAX_TIME_RETRIES,
            "max_call_retries": MAX_CALL_RETRIES,
            "time_match_threshold": TIME_MATCH_THRESHOLD,
            "snap_time_to_video_fps_grid": SNAP_TIME_TO_VIDEO_FPS_GRID,
            "cuda_memory_reserve_mib": CUDA_MEMORY_RESERVE_MIB,
...
def update_summary_for_video(
    ...
    *,
    ...
    if elapsed_seconds is not None:
        entry["elapsed_seconds"] = round(float(elapsed_seconds), 4)
    if reason:
        entry["reason"] = reason

    if status == "skipped_existing":
        summary["skipped_existing_count"] += 1
        summary["last_video"] = entry
        return
    ...
    if status == "success":
        ...
        summary.setdefault("failed_videos", []).append(dict(entry))
    summary["last_video"] = entry
...
def parse_accident_time_value(value: Any) -> Optional[float]:
    try:
        return float(value)
    except (TypeError, ValueError):
        return None

def collect_zero_time_retry_targets(
    csv_path: str,
    fieldnames: List[str],
    ...
    if not os.path.exists(csv_path):
        return retry_paths, kept_paths, kept_rows
    ...
    with open(csv_path, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if not rel_path:
                continue
            accident_time = parse_accident_time_value(row.get("accident_time"))
            if accident_time is not None and abs(accident_time) < 1e-9:
                retry_paths.add(rel_path)
                continue
            kept_paths.add(rel_path)
            kept_rows.append({name: row.get(name, "") for name in fieldnames})

    return retry_paths, kept_paths, kept_rows

def write_prediction_rows(csv_path: str, rows: List[Dict[str, Any]], fieldnames: List[str]) -> None:
    """submission.csv 전체를 다시 써서 특정 행들을 제거하거나 정리합니다."""
...
def main():
    ...
    run_summary = build_run_summary(len(metadata_rows), active_video_dir, metadata_rows)
    ...
    if comp_error:
        ...
        run_summary["status"] = "failed_before_run"
        run_summary["model_load"] = {
            "status": "failed",
            "elapsed_seconds": None,
            "error": comp_error,
        }
        run_summary["finished_at"] = current_utc_iso()
        run_summary["total_elapsed_seconds"] = round(time.perf_counter() - total_run_start, 4)
        write_run_summary(run_summary)
        return
    ...
    existing_paths = set()
    retry_zero_time_paths = set()

    if RETRY_ZERO_TIME_ONLY:
        retry_zero_time_paths, existing_paths, kept_prediction_rows = collect_zero_time_retry_targets(
            PREDICTION_PATH,
            prediction_fieldnames,
        )
        if os.path.exists(PREDICTION_PATH):
            write_prediction_rows(PREDICTION_PATH, kept_prediction_rows, prediction_fieldnames)
        for retry_path in retry_zero_time_paths:
            raw_log_path = get_raw_log_path(retry_path)
            if os.path.exists(raw_log_path):
                os.remove(raw_log_path)
        metadata_rows = [m for m in metadata_rows if m.get("path") in retry_zero_time_paths]
        print(f"[retry] accident_time=0.0 행 {len(retry_zero_time_paths)}개만 다시 추론합니다.")
    ...
    run_summary["model_load"] = {
        "status": "loaded",
        "elapsed_seconds": round(model_load_elapsed, 4),
        "error": None,
    }
    run_summary["initialization_elapsed_seconds"] = round(init_elapsed, 4)
    write_run_summary(run_summary)
    ...
    for idx, meta in enumerate(metadata_rows, start=1):
        ...
        if not rel_path:
            update_summary_for_video(run_summary, rel_path, status="failure", stage="input", outcome="메타데이터 path 누락", row_index=idx)
            write_run_summary(run_summary)
            continue

        if SKIP_EXISTING and rel_path in existing_paths:
            update_summary_for_video(run_summary, rel_path, status="skipped_existing", stage="skip", outcome="기존 결과 존재", row_index=idx)
            write_run_summary(run_summary)
            continue
        ...
        if not abs_video_path:
            ...
            update_summary_for_video(run_summary, rel_path, status="failure", stage="input", outcome="비디오 파일 없음", row_index=idx)
            write_run_summary(run_summary)
            continue
        ...
        try:
            ...
            for t_attempt in range(1, MAX_TIME_RETRIES + 1):
                ...
                if temp_frame_index is not None:
                    if temp_frame_index in failed_indices:
                        print(f"  -> [time] 프레임 인덱스 {temp_frame_index}는 이미 실패했던 값이라 무시합니다.")
                        continue

                    candidate_time = float(sampled_video["timestamps"][temp_frame_index])
                    if abs(candidate_time) < 1e-9:
                        if temp_frame_index not in failed_indices:
                            failed_indices.append(temp_frame_index)
                        if t_attempt < MAX_TIME_RETRIES:
                            print("  -> [time] accident_time=0.0 예측 감지: 다시 추론 시도합니다.")
                            continue
                        time_frame_index = temp_frame_index
                        accident_time = 0.0
                        time_forced_zero = True
                        break

                    time_frame_index = temp_frame_index
                    accident_time = candidate_time
                    break
            ...
            if time_forced_zero:
                prediction_row = {"path": rel_path, "accident_time": 0.0, "center_x": 0.0, "center_y": 0.0, "type": "single"}
                append_prediction_row(PREDICTION_PATH, prediction_row, prediction_fieldnames)
                existing_paths.add(rel_path)
                video_outcome = "사고 시점 0.0초 fallback 저장"
                failure_stage = "time_fallback"
                print(f"  -> submission.csv fallback 저장 완료: {rel_path}")
                continue
            ...
            # --- [Step 2: 핵심 프레임 추출] ---
            sampled_frame_fallback = None
            if 0 <= time_frame_index < len(sampled_video["frames"]):
                try:
                    sampled_frame_fallback = {
                        "frame": cv2.cvtColor(np.array(sampled_video["frames"][time_frame_index]), cv2.COLOR_RGB2BGR),
                        "fps": sampled_video["raw_fps"],
                        "frame_index": int(sampled_video["frame_indices"][time_frame_index]),
                    }
                except Exception: pass

            extracted = extract_frame_at_time(abs_video_path, accident_time, meta, fps_override=sampled_video["raw_fps"])
            if not extracted and sampled_frame_fallback:
                extracted = sampled_frame_fallback
                print("  -> [frame] 원본 seek 실패: sampled frame fallback 사용")

            if not extracted:
                prediction_row = {"path": rel_path, "accident_time": 0.0, "center_x": 0.0, "center_y": 0.0, "type": "single"}
                append_prediction_row(PREDICTION_PATH, prediction_row, prediction_fieldnames)
                existing_paths.add(rel_path)
                video_outcome = "프레임 추출 실패 -> 0 fallback 저장"
                failure_stage = "frame"
                print("  -> [오류] 프레임 추출 실패: accident_time=0.0 fallback으로 저장합니다.")
                continue
            ...
            update_summary_for_video(run_summary, rel_path, status="success", stage="completed", outcome="성공", row_index=idx, elapsed_seconds=time.perf_counter() - video_start)
        except Exception as exc:
            failure_stage = "exception"
            failure_reason = f"{type(exc).__name__}: {exc}"
            video_outcome = f"예외 발생: {failure_reason}"
            update_summary_for_video(run_summary, rel_path, status="failure", stage=failure_stage, outcome=video_outcome, reason=failure_reason, row_index=idx)
        finally:
            write_run_summary(run_summary)
            video_elapsed = time.perf_counter() - video_start
            print(f"  -> [time] 영상 처리 시간: {format_elapsed(video_elapsed)} (status={video_outcome})")
