...
OUTPUT_DIR = "/root/Desktop/workspace/ja/qwen_inference3-4bit-sam3"
...
FEATURE_ROOT_DIR = "/root/Desktop/workspace/ja/_try1/qwen_inference3-4bit/sam3_features"
...
MIN_PYTHON = (3, 8)
MIN_TRANSFORMERS = (4, 46) # Qwen3.5/Qwen3-VL 지원 버전

VALID_TYPES = {"rear-end", "head-on", "sideswipe", "t-bone", "single"}
MAX_VIDEO_RETRIES = 2
MAX_NEW_TOKENS = 15000000
TEMPERATURE = 0.0
TOP_P = 1.0
...
# SAM3 관련 설정
SAM3_ACCIDENT_CLASSES = {0, 1, 2, 3}
SAM3_DIR = "/root/Desktop/workspace/ja/temp3"
...
def build_secondary_feature_note(
    ...
    return (
        ...
        "- The following cues are derived from object tracking. Consider these cues deeply while also giving appropriate weight to the visual evidence. Do not follow them blindly if they clearly contradict the visible image.\n"
        f"- {compact}"
    )
...
def build_quantization_config() -> Tuple[Optional[BitsAndBytesConfig], Optional[str]]:
    return None, "4-bit quantization disabled."
...
def load_sam3_data(video_stem: str) -> List[Dict[str, Any]]:
    """Loads SAM3 tracking CSV data for the given video."""
    csv_path = os.path.join(SAM3_DIR, f"{video_stem}_sam3_tracks.csv")
    if not os.path.exists(csv_path):
        return []
    ...
    try:
        ...
        for r in rows:
            # 기본 파싱
            item = {
                "frame_idx": safe_int(r.get("frame_idx")),
                "track_id": safe_int(r.get("track_id")),
                "class_id": safe_int(r.get("class_id")),
                "class_name": r.get("class_name", "").strip(),
                "confidence": safe_float(r.get("confidence")),
                "bbox": [safe_float(r.get("x1")), safe_float(r.get("y1")), 
                         safe_float(r.get("x2")), safe_float(r.get("y2"))]
            }
            parsed_rows.append(item)
        return parsed_rows
    except Exception as e:
        print(f"  -> [SAM3] Error loading {csv_path}: {e}")
        return []


def get_sam3_summary(sam3_rows: List[Dict[str, Any]]) -> str:
    """Summarizes SAM3 accident detections for prompt context."""
    if not sam3_rows:
        return ""
    
    accident_tracks = [r for r in sam3_rows if r["class_id"] in SAM3_ACCIDENT_CLASSES]
    if not accident_tracks:
        return "\nSAM3 detected no specific accident-class objects in this video."
    
    # 트랙별 요약
    summary = "\nSAM3 Motion Tracking Analysis (Suspected Accident Objects):"
    tracks_seen = set()
    for r in accident_tracks:
        tid = r["track_id"]
        if tid in tracks_seen: continue
        tracks_seen.add(tid)
        
        # 해당 트랙의 전체 신뢰도/프레임 수 등 집계 가능하나 일단 단순화
        summary += f"\n- Object ID {tid} was classified as '{r['class_name']}' with {r['confidence']:.2f} confidence."
...
- Keep reasoning extremely concise (1-2 sentences) so the final JSON is output BEFORE hitting the length limit.
- Output JSON only.
- No reasoning.
- No analysis.
- No markdown.
...
def get_system_prompt_for_stage(stage: str, sam3_summary: str = "") -> str:
    ...
    if sam3_summary:
        common += f"\n\nContextual Information (SAM3 Analysis): {sam3_summary}"
            ...
            messages = [
                {
                    "role": "system",
                    "content": [
                        {"type": "text", "text": get_system_prompt_for_stage(stage, sam3_summary)},
                    ],
                },
                {
                    "role": "user",
...
def _run_single_video_inference_attempt(
    ...
    # SAM3 연동
    sam3_rows = load_sam3_data(video_stem)
    sam3_summary = get_sam3_summary(sam3_rows)
