...
# /opt/conda/envs/qwen35/bin/python qwen_all_inference.py
...
OUTPUT_DIR = "/root/Desktop/workspace/ja/qwen_inference"

# TEST_MODE = False
TEST_MODE = True
TEST_VIDEO_DIR = "/root/Desktop/workspace/ja/CVPR-Kaggle-files/Detection_Dataset/temp"
TEST_LIMIT = 1

FEATURE_ROOT_DIR = "/root/Desktop/workspace/ja/CVPR-Kaggle-files/Detection_Dataset/test_features"
...
TIME_CLIP_DIR = os.path.join(OUTPUT_DIR, "time_clips")
...
TIME_MAX_NEW_TOKENS = 32
LOCATION_MAX_NEW_TOKENS = 48
TYPE_MAX_NEW_TOKENS = 24
DEFAULT_MAX_RETRIES = 1
CUDA_MEMORY_RESERVE_MIB = 256

TIME_TOP_PAIR_K = 5
TIME_TOP_SINGLE_K = 5
TIME_MAX_CLIP_CANDIDATES = 4
TIME_CLIP_WINDOW_BEFORE_SEC = 2.0
TIME_CLIP_WINDOW_AFTER_SEC = 1.5
TIME_CLIP_MIN_DURATION_SEC = 3.0
TIME_CANDIDATE_MERGE_SEC = 1.0
TIME_CLIP_FPS = 4
...
def build_model_load_config() -> Tuple[Dict[str, Any], List[str]]:
    ...
    if manual_device:
        ...
        return {"device_map": manual_device, "torch_dtype": "auto"}, notes
    ...
    if not max_memory:
        ...
        return {"device_map": "cuda:0", "torch_dtype": "auto"}, notes

    if enable_cpu_offload:
        max_memory["cpu"] = "64GiB"
        notes.append("CPU offload enabled via QWEN_ENABLE_CPU_OFFLOAD=1")

    return {"device_map": "auto", "max_memory": max_memory, "torch_dtype": "auto"}, notes
...
def compute_time_clip_bounds(
    center_time: float,
    video_duration: Optional[float],
    before_sec: float = TIME_CLIP_WINDOW_BEFORE_SEC,
    after_sec: float = TIME_CLIP_WINDOW_AFTER_SEC,
    min_duration_sec: float = TIME_CLIP_MIN_DURATION_SEC,
) -> Tuple[float, float]:
    start_time = max(0.0, center_time - before_sec)
    ...
    if end_time - start_time < min_duration_sec:
        deficit = min_duration_sec - (end_time - start_time)
        start_time = max(0.0, start_time - deficit / 2.0)
        end_time = start_time + min_duration_sec
        if video_duration is not None and video_duration > 0 and end_time > video_duration:
            end_time = video_duration
            start_time = max(0.0, end_time - min_duration_sec)

    if end_time <= start_time:
        end_time = start_time + max(min_duration_sec, 0.5)
        if video_duration is not None and video_duration > 0:
            end_time = min(end_time, video_duration)
            start_time = max(0.0, min(start_time, end_time - 0.5))

    return start_time, end_time


def build_time_clip_candidates(feature_signals: Dict[str, Any], meta: Dict[str, str]) -> List[Dict[str, Any]]:
    duration = safe_float(meta.get("duration"))
    raw_candidates: List[Dict[str, Any]] = []

    for row in feature_signals.get("top_pairs", []):
        ...
        raw_candidates.append(
            {
                "source": "pair",
                "time_sec": time_sec,
                "score": safe_float(row.get("pair_risk_score"), 0.0) or 0.0,
                "label": str(row.get("pair") or "unknown"),
            }
        )

    for row in feature_signals.get("top_singles", []):
        time_sec = safe_float(row.get("time_sec"))
        if time_sec is None:
            continue
        raw_candidates.append(
            {
                "source": "single",
                "time_sec": time_sec,
                "score": safe_float(row.get("single_risk_score"), 0.0) or 0.0,
                "label": f"track_{row.get('track_id')}",
            }
        )
    ...
    for candidate in sorted(raw_candidates, key=lambda item: item["time_sec"]):
        if not merged_candidates:
            merged_candidates.append(candidate)
            continue
        previous = merged_candidates[-1]
        if candidate["time_sec"] - previous["time_sec"] <= TIME_CANDIDATE_MERGE_SEC:
    ...
    selected = sorted(
        merged_candidates,
        key=lambda item: (-item["score"], item["time_sec"]),
    )[:TIME_MAX_CLIP_CANDIDATES]

    clip_candidates: List[Dict[str, Any]] = []
    for candidate in sorted(selected, key=lambda item: item["time_sec"]):
        start_time, end_time = compute_time_clip_bounds(candidate["time_sec"], duration)
        clip_candidates.append(
            {
                **candidate,
                "clip_start_sec": round(start_time, 3),
                "clip_end_sec": round(end_time, 3),
                "clip_duration_sec": round(max(end_time - start_time, 0.0), 3),
            }
        )
...
def extract_time_clip(video_path: str, clip_path: str, start_time: float, duration_sec: float) -> Optional[str]:
    if duration_sec <= 0:
        return "Clip duration must be positive."
    ...
    if completed.returncode != 0:
        ...
        return error_text or f"ffmpeg failed with exit code {completed.returncode}"
    if not os.path.exists(clip_path):
        return "ffmpeg reported success but clip file was not created."
    return None
...
def build_type_object_context(
    ...
    selection = select_objects_at_predicted_impact(feature_index, frame_index, center_x, center_y, meta)
    object_summaries: List[Dict[str, Any]] = []
    for track_id in selection["selected_track_ids"][:2]:
        if track_id is None:
            continue
        summary = summarize_object_trajectory(feature_index, int(track_id), frame_index)
        if summary is not None:
            object_summaries.append(summary)

    pair_summary = None
    if len(selection["selected_track_ids"]) >= 2:
        pair_summary = summarize_pair_context(
            feature_index,
            [int(selection["selected_track_ids"][0]), int(selection["selected_track_ids"][1])],
            frame_index,
        )

    heuristic_interpretation = build_heuristic_interpretation(object_summaries, pair_summary)
    return {
        "predicted_impact_location": {"center_x": center_x, "center_y": center_y},
        "selection": selection,
...
def format_type_object_feature_text(feature_index: Dict[str, Any], type_context: Dict[str, Any]) -> str:
    ...
    lines = [
        "Trajectory-centric type cues:",
        (
            "predicted_impact_location="
            f"({type_context['predicted_impact_location']['center_x']:.3f}, "
            f"{type_context['predicted_impact_location']['center_y']:.3f})"
        ),
    ]

    selection = type_context["selection"]
    lines.append(
        "selected_object_ids="
        f"{selection.get('selected_track_ids', [])} "
        f"match_mode={selection.get('match_mode') or 'none'} "
        f"matched_frame={selection.get('matched_frame_idx')}"
    )

    if selection.get("selected_objects"):
        for obj in selection["selected_objects"][:2]:
            lines.append(
                "  "
                f"matched_track={obj.get('track_id')} class={obj.get('class_name') or 'unknown'} "
                f"center={fmt_point(obj.get('center_norm'))} inside_bbox={obj.get('inside_bbox')} "
                f"distance_to_point={fmt_num(obj.get('distance_to_point'))}"
            )

    if type_context["object_summaries"]:
        lines.append("Object trajectory summaries:")
        for obj in type_context["object_summaries"][:2]:
            lines.append(
                "  "
                f"track={obj.get('track_id')} class={obj.get('class_name') or 'unknown'} "
                f"start={fmt_point(obj.get('start_center_norm'))} "
                f"impact={fmt_point(obj.get('impact_center_norm'))} "
                f"end={fmt_point(obj.get('end_center_norm'))} "
                f"traj_dir={fmt_num(obj.get('traj_direction_at_impact'))} "
                f"pre_vel={fmt_num(obj.get('mean_velocity_pre_impact'))} "
                f"accel={fmt_num(obj.get('acceleration_at_impact'))} "
                f"jerk={fmt_num(obj.get('jerk_at_impact'))} "
                f"dir_change={fmt_num(obj.get('direction_change_at_impact'))} "
                f"curvature={fmt_num(obj.get('curvature_near_impact'))} "
                f"movement_span_px={fmt_num(obj.get('movement_span_px'))}"
            )
    else:
        lines.append("Object trajectory summaries: none")

    pair_summary = type_context.get("pair_summary")
    if pair_summary is not None:
        lines.append(
            "pair_relation="
            f"pair={pair_summary.get('pair')} "
            f"path_relation={pair_summary.get('path_relation')} "
            f"orientation_relation={pair_summary.get('orientation_relation')} "
            f"distance={fmt_num(pair_summary.get('distance_at_impact'))} "
            f"min_distance={fmt_num(pair_summary.get('min_distance_in_window'))} "
            f"ttc={fmt_num(pair_summary.get('ttc_at_impact'))} "
            f"min_ttc={fmt_num(pair_summary.get('min_ttc_in_window'))} "
            f"approach={fmt_num(pair_summary.get('approach_speed_at_impact'))} "
            f"v_rel={fmt_num(pair_summary.get('v_rel_at_impact'))} "
            f"trajectory_angle_diff={fmt_num(pair_summary.get('trajectory_angle_diff_at_impact'), 1)} "
            f"relative_angle={fmt_num(pair_summary.get('relative_angle_at_impact'), 1)}"
        )
    else:
        lines.append("pair_relation=none")

    lines.append("heuristic_interpretation:")
    for hint in type_context["heuristic_interpretation"]:
        lines.append(f"  {hint}")
...
def build_time_prompt(metadata: Dict[str, str], feature_text: str = "") -> str:
    ...
    feature_block = feature_text.strip() if feature_text.strip() else "No auxiliary tracking feature cues were available."
...
How to use the tracking cues:
- These tracker-derived cues are noisy and secondary. The video itself is the primary evidence.
- ttc below 1.0, rapidly shrinking distance, and high v_rel indicate a likely multi-object collision window.
- Large negative acceleration, large velocity_drop, and large absolute jerk indicate abrupt impact dynamics.
- Large direction_change and sustained curvature indicate spinning, bouncing, or loss of control.
- If the video contradicts the tracking cues, trust the video.
...
def build_time_clip_prompt(
    ...
    prompt = f"""
You are an expert traffic accident analyst looking at a SHORT CCTV clip extracted from a longer video.

The clip you are watching covers this ORIGINAL video time range only:
- clip_start_sec: {clip_start_sec:.3f}
- clip_end_sec: {clip_end_sec:.3f}

Your task is to decide whether the FIRST clear traffic accident begins within this clip window.
...
Instructions:
1. Analyze ONLY this clip window.
2. If a clear traffic accident begins inside this clip, output has_accident=true.
3. If has_accident=true, accident_time must be the ABSOLUTE time in the original full video, not the clip-relative time.
4. If no clear accident begins in this clip, output has_accident=false and accident_time=null.
5. The accident start time means the first frame where physical contact begins, or collision becomes clearly unavoidable and immediate.
...
How to use the tracking cues:
- Use pair_center_hint or matched object centers only as priors, then refine from the image.
- Tracking cues are noisy and secondary. The image is the primary evidence.
- If the image contradicts the tracking cues, trust the image.
...
def build_type_prompt(metadata: Dict[str, str], accident_time: float, feature_text: str = "") -> str:
    region = metadata.get("region", "")
    scene_layout = metadata.get("scene_layout", "")
    weather = metadata.get("weather", "")
    day_time = metadata.get("day_time", "")
...
How to use the tracking cues:
- First analyze the selected object ids near the predicted impact location and use their trajectories as the primary cue.
- The image frame is secondary verification for that trajectory-based hypothesis.
- trajectory_angle_diff is the primary path-relation cue.
- relative_angle is the secondary collision-orientation cue.
- same-direction plus rear-end-like usually indicates rear-end.
- same-direction with side overlap or parallel lateral contact usually indicates sideswipe.
- crossing plus side-impact-like usually indicates t-bone.
- opposite-direction plus head-on-like usually indicates head-on.
- If pair evidence is weak but a single matched object shows strong braking, jerk, or rotation, strongly consider single.
- If the image contradicts the tracking cues, trust the image.
...
2. Prioritize the trajectories of the matched object ids around the predicted impact location.
3. Choose exactly one type from:
   ["rear-end", "head-on", "sideswipe", "t-bone", "single"].
4. If uncertain, choose the single best guess.
...
def validate_time_clip_prediction(
    result: Dict[str, Any],
    meta: Dict[str, str],
    clip_start_sec: float,
    clip_end_sec: float,
) -> Optional[Dict[str, Any]]:
    has_accident = parse_bool_flag(result.get("has_accident"))
    if has_accident is None:
        accident_time_value = result.get("accident_time")
        has_accident = accident_time_value is not None and str(accident_time_value).strip().lower() not in {"", "null", "none"}

    if not has_accident:
        return {"has_accident": False, "accident_time": None}
    ...
    tolerance_sec = 0.35
    if accident_time < clip_start_sec - tolerance_sec or accident_time > clip_end_sec + tolerance_sec:
        return None

    return {"has_accident": True, "accident_time": accident_time}
...
def call_qwen_for_media_single(
    ...
    try:
        ...
        print(f"  -> [{stage}] preprocessing failed: {exc}")
        return None
...
def build_feature_context_payload(feature_bundle: Dict[str, Any]) -> Dict[str, Any]:
    return {
        ...
        "time_stage_mode": None,
        "time_clip_candidates": [],
        "selected_time_clip": None,
...
def build_type_object_context_summary(type_context: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    return {
        ...
        "pair_summary": type_context.get("pair_summary"),
        "object_summaries": type_context.get("object_summaries", []),
        "heuristic_interpretation": type_context.get("heuristic_interpretation", []),
    }
...
def run_single_video_inference(
    ...
    clip_dir_for_video = os.path.abspath(os.path.join(TIME_CLIP_DIR, video_stem))
    ...
    feature_context["time_clip_candidates"] = time_clip_candidates
    ...
    try:
        ...
        log_feature_prompt_input(
            ...
            feature_index,
            time_feature_text,
            extra={"time_clip_candidates": time_clip_candidates},
        )

        accident_time: Optional[float] = None
        selected_time_clip: Optional[Dict[str, Any]] = None

        if time_clip_candidates:
            os.makedirs(clip_dir_for_video, exist_ok=True)
            print(f"  -> [time] evaluating {len(time_clip_candidates)} candidate clips")
            for clip_index, clip_candidate in enumerate(time_clip_candidates, start=1):
                clip_filename = (
                    f"{video_stem}_candidate{clip_index:02d}_"
                    f"{clip_candidate['clip_start_sec']:.3f}_{clip_candidate['clip_end_sec']:.3f}.mp4"
                )
                clip_path = os.path.abspath(os.path.join(clip_dir_for_video, clip_filename))
                clip_error = extract_time_clip(
                    abs_video_path,
                    clip_path,
                    float(clip_candidate["clip_start_sec"]),
                    float(clip_candidate["clip_duration_sec"]),
                )
                clip_stage = f"time_clip_{clip_index}"
                if clip_error is not None:
                    ...
                    print(f"  -> [{clip_stage}] clip extraction failed: {clip_error}")
                    continue

                raw_time_clip = call_qwen_for_media_single(
                    model=model,
                    processor=processor,
                    media_type="video",
                    media_path=clip_path,
                    prompt=build_time_clip_prompt(
                        meta,
                        time_feature_text,
                        float(clip_candidate["clip_start_sec"]),
                        float(clip_candidate["clip_end_sec"]),
                    ),
                    rel_path=rel_path,
                    stage=clip_stage,
                    raw_log_path=raw_log_path,
                    max_new_tokens=TIME_MAX_NEW_TOKENS,
                )
                if raw_time_clip is None:
                    continue

                validated_time_clip = validate_time_clip_prediction(
                    raw_time_clip,
                    meta,
                    float(clip_candidate["clip_start_sec"]),
                    float(clip_candidate["clip_end_sec"]),
                )
                if validated_time_clip is None:
                    append_raw_log(
                        raw_log_path,
                        {
                            "path": rel_path,
                            "stage": clip_stage,
                            "clip_candidate": clip_candidate,
                            "validation_error": "Invalid clip time prediction schema or out-of-window time.",
                            "raw_prediction": raw_time_clip,
                        },
                    )
                    print(f"  -> [{clip_stage}] invalid clip time prediction.")
                    continue

                if not validated_time_clip["has_accident"]:
                    append_raw_log(
                        raw_log_path,
                        {
                            "path": rel_path,
                    ...
                    print(f"  -> [{clip_stage}] no accident detected in candidate clip.")
                    continue

                accident_time = float(validated_time_clip["accident_time"])
                selected_time_clip = {
                    **clip_candidate,
                    "clip_path": clip_path,
                    "selection_stage": clip_stage,
                }
                feature_context["time_stage_mode"] = "clip"
                feature_context["selected_time_clip"] = selected_time_clip
                print(f"  -> [{clip_stage}] selected accident_time={accident_time:.4f}")
                break

        if accident_time is None:
            feature_context["time_stage_mode"] = "full_video_fallback"
            print("  -> [time] clip search missed; falling back to full video inference")
            raw_time = call_qwen_for_media_single(
                model=model,
                processor=processor,
                media_type="video",
                media_path=abs_video_path,
                prompt=build_time_prompt(meta, time_feature_text),
                rel_path=rel_path,
                stage="time",
                raw_log_path=raw_log_path,
                max_new_tokens=TIME_MAX_NEW_TOKENS,
            ...
            if accident_time is None:
                return finalize_failure("time", f"Invalid time prediction schema: {raw_time}")
            feature_context["selected_time_clip"] = {"selection_stage": "time", "mode": "full_video_fallback"}

        print(f"  -> [time] parsed accident_time={accident_time:.4f}")

        print("  -> [frame] extracting frame")
        ...
        raw_type = call_qwen_for_media_single(
            ...
            stage="type",
            raw_log_path=raw_log_path,
            max_new_tokens=TYPE_MAX_NEW_TOKENS,
        )
        if raw_type is None:
            return finalize_failure(
                "type",
...
def main() -> None:
    ...
    os.makedirs(TIME_CLIP_DIR, exist_ok=True)
