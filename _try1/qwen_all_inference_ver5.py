...
import torch
import transformers
from extract_sam3_tracking_features import build_features_for_video
...
TEST_VIDEO_BASENAME = "-6SQSDj8cYU_00.mp4"
TEST_LIMIT = 0
...
SAM3_FEATURE_ROOT_DIR = os.path.join(OUTPUT_DIR, "sam3_features")
...
def filter_metadata_for_test(
    ...
    available_files = {
        ...
        and name.lower().endswith((".mp4", ".avi", ".mov", ".mkv"))
    }
    if TEST_VIDEO_BASENAME:
        available_files = {name for name in available_files if name == TEST_VIDEO_BASENAME}
...
def load_feature_bundle(video_stem: str) -> Dict[str, Any]:
    ...
    sam3_paths = None
    try:
        sam3_paths = build_features_for_video(
            f"{video_stem}.mp4",
            sam3_input_dir=Path(TEST_VIDEO_DIR),
            output_dir=Path(SAM3_FEATURE_ROOT_DIR),
            overwrite=False,
        )
    except Exception as exc:
        bundle["warning"] = f"SAM3 feature generation failed: {exc}"

    if sam3_paths is not None:
        intra_path = os.path.abspath(str(sam3_paths[0]))
        inter_path = os.path.abspath(str(sam3_paths[1]))
        bundle["intra_path"] = intra_path
        bundle["inter_path"] = inter_path
        bundle["warning"] = "Loaded feature CSVs generated from SAM3 tracking outputs."
        bundle["source"] = "sam3_generated"
