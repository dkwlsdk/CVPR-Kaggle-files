#!/opt/conda/envs/qwen35/bin/python
import csv
import json
import math
import os
...
# /opt/conda/envs/qwen35/bin/python qwen_all_inference_ver4.py
...
OUTPUT_DIR = "/root/Desktop/workspace/ja/qwen_inference_ver4_output"
...
MIN_PYTHON = (3, 10)
MIN_TRANSFORMERS = (4, 45)  # Qwen2-VL requires transformers>=4.45.0
DEFAULT_BACKEND = (os.environ.get("QWEN_BACKEND", "vllm").strip().lower() or "vllm")
ACTIVE_BACKEND = "transformers"
DEFAULT_VLLM_GPU_MEMORY_UTILIZATION = 0.9
DEFAULT_VLLM_MAX_NUM_SEQS = 1
DEFAULT_VLLM_LIMIT_MM_IMAGES = 8
DEFAULT_VLLM_LIMIT_MM_VIDEOS = 1
...
def resolve_inference_backend() -> Tuple[str, List[str]]:
    backend = DEFAULT_BACKEND
    notes: List[str] = []

    if backend not in {"vllm", "transformers"}:
        notes.append(f"Unknown QWEN_BACKEND={backend!r}; falling back to vllm.")
        backend = "vllm"

    if backend == "vllm" and (LLM is None or SamplingParams is None):
        notes.append("vLLM requested but not installed; falling back to transformers.")
        backend = "transformers"

    if backend == "vllm" and not torch.cuda.is_available():
        notes.append("CUDA unavailable; falling back to transformers backend.")
        backend = "transformers"

    return backend, notes


def build_transformers_model_load_config() -> Tuple[Dict[str, Any], List[str]]:
    notes: List[str] = []
    manual_device = os.environ.get("QWEN_TARGET_DEVICE", "").strip()
    enable_cpu_offload = os.environ.get("QWEN_ENABLE_CPU_OFFLOAD", "").strip() == "1"
    enable_quantization = os.environ.get("QWEN_ENABLE_QUANTIZATION", "1").strip() == "1"
    ...
    config: Dict[str, Any] = {"device_map": "auto", "max_memory": max_memory, "torch_dtype": "auto"}

    if enable_quantization:
        notes.append("4-bit quantization enabled via BitsAndBytesConfig.")
...
def build_vllm_model_load_config(video_dir: str) -> Tuple[Dict[str, Any], List[str]]:
    notes: List[str] = []
    visible_gpu_count = torch.cuda.device_count() if torch.cuda.is_available() else 0
    tensor_parallel_size = int(
        os.environ.get(
            "VLLM_TENSOR_PARALLEL_SIZE",
            str(max(1, visible_gpu_count)),
        )
    )
    gpu_memory_utilization = clamp(
        safe_float(
            os.environ.get("VLLM_GPU_MEMORY_UTILIZATION"),
    ...
    max_num_seqs = max(
        1,
        safe_int(os.environ.get("VLLM_MAX_NUM_SEQS"), DEFAULT_VLLM_MAX_NUM_SEQS) or DEFAULT_VLLM_MAX_NUM_SEQS,
    )
    limit_mm_images = max(
        1,
        safe_int(os.environ.get("VLLM_LIMIT_MM_IMAGES"), DEFAULT_VLLM_LIMIT_MM_IMAGES)
        or DEFAULT_VLLM_LIMIT_MM_IMAGES,
    )
    limit_mm_videos = max(
        1,
        safe_int(os.environ.get("VLLM_LIMIT_MM_VIDEOS"), DEFAULT_VLLM_LIMIT_MM_VIDEOS)
        or DEFAULT_VLLM_LIMIT_MM_VIDEOS,
    )

    media_root_override = os.environ.get("VLLM_ALLOWED_LOCAL_MEDIA_PATH", "").strip()
    if media_root_override:
        allowed_local_media_path = os.path.abspath(media_root_override)
        notes.append(f"Using VLLM_ALLOWED_LOCAL_MEDIA_PATH override: {allowed_local_media_path}")
    else:
        allowed_local_media_path = os.path.commonpath(
            [
                os.path.abspath(video_dir),
                os.path.abspath(FRAME_DIR),
            ]
        )
        notes.append(f"Derived allowed_local_media_path={allowed_local_media_path}")
    ...
    for device_index in range(visible_gpu_count):
        ...
        if free_bytes is None:
            notes.append(f"cuda:{device_index} ({gpu_name}): free memory unavailable")
            continue
        notes.append(f"cuda:{device_index} ({gpu_name}): free={free_bytes / (1024**3):.2f} GiB")

    max_model_len = safe_int(os.environ.get("VLLM_MAX_MODEL_LEN"))
    config: Dict[str, Any] = {
        "model": MODEL_NAME,
...
def ensure_runtime_compatibility(backend: str) -> Optional[str]:
    ...
    if backend == "vllm" and (LLM is None or SamplingParams is None):
        return "vllm is required for QWEN_BACKEND=vllm but is not installed in the current environment."
...
def build_time_prompt(metadata: Dict[str, str], feature_text: str = "", attempt: int = 0, failed_times: List[float] = None) -> str:
    ...
    if attempt > 0:
        ...
        if failed_times:
            retry_note += f" You previously estimated accident times at {failed_times}, but those were determined to be incorrect or inconsistent with the visual evidence. DO NOT repeat these same estimations. Analyze the entire video again carefully with a fresh perspective to find a DIFFERENT, more definite collision segment."
        else:
...
def generate_vllm_text(
    model,
    processor: AutoProcessor,
    messages: List[Dict[str, Any]],
    prefix: str = "",
) -> str:
    request_input = build_vllm_request_input(processor, messages, prefix=prefix)
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if ACTIVE_BACKEND == "vllm":
                collected_text = generate_vllm_text(
                    model=model,
                    processor=processor,
                    messages=messages,
                    prefix=prefix,
                )
                print(f"  -> [{stage}] model output: {collected_text}", flush=True)
            else:
...
def main() -> None:
    ...
    backend, backend_notes = resolve_inference_backend()
    ACTIVE_BACKEND = backend
    print(f"Inference backend: {ACTIVE_BACKEND}")
    ...
    compatibility_error = ensure_runtime_compatibility(ACTIVE_BACKEND)
    ...
    try:
        ...
        if ACTIVE_BACKEND == "vllm":
            model_load_config, load_notes = build_vllm_model_load_config(effective_video_dir)
            for note in load_notes:
                print(f"Model load config: {note}")
            model = LLM(**model_load_config)
            if vllm is not None:
                print(f"Using vLLM version: {getattr(vllm, '__version__', 'unknown')}")
        else:
            model_load_config, load_notes = build_transformers_model_load_config()
            for note in load_notes:
                print(f"Model load config: {note}")
